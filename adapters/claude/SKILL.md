---
name: loupe
description: Use when asked to hand off work for review, take a review request, respond to a verdict, or close a round with `loupe`. The governing config is `review.toml` at the repo root OR a user-level `~/.config/loupe/<repo-id>.toml` — a repository with nothing in-tree can still be under review, so do not require an in-tree file before acting. Carries the literal commands for the author and reviewer sides; the envelope stamp says which side you hold.
---

# loupe — review procedure (Claude Code adapter, GENERATED)

GENERATED by `loupe render-adapters` from one source (review/adapters.py) at tool version 0.7.0 — do not edit; regenerate. `loupe render-adapters --check` guards it.

Install: `~/.claude/skills/loupe/SKILL.md` (conventional; first installed here 2026-08-16).
