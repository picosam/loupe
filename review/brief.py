...
#: The emitter's in-scope line (0.25.0), and its counted claim notices.
_IN_SCOPE = re.compile(
    r"^In scope: (\d+ files?, \d+ insertions?, \d+ deletions?)", re.M)
_CLAIM_NOTICE = re.compile(r"^Notice — (.+? \(\d+\)):", re.M)
...
def request_precis(parsed, ledger=None, full: bool = False) -> str:
    ...
    # 0.25.0: the claim's own notices (objectives that map nothing, carried
    # findings this machine cannot verify, fix commits outside the span…),
    # each already counted on the envelope's face by the emitter.
    for label in _CLAIM_NOTICE.findall(claim):
    ...
    # The in-scope count beside the machine total (0.25.0), when the
    # envelope carries one: the span less what is generated by declaration
    # and what the claim excludes.
    if changed and (m := _IN_SCOPE.search(claim)):
        changed += f"; in scope: {m.group(1)}"
...
def target_taxonomy(cfg, sha: str | None):
    """`(governing, None)` — the configuration at the verdict's TARGET — or
    `(None, why)` when that authority cannot be resolved here.

    An adopter's open item (2026-09-20): the précis compared severities
    against a built-in `("Blocker", "High")`, case-sensitively, and no caller
    passed a taxonomy — so a repository declaring `blocking = ["high"]` read
    "7 findings, of which 0 block" with its `high` finding filed under
    Non-blocking, about a verdict its own validator judged by the declared
    set. The set is now the TARGET's, resolved by the one authority
    `validate --from-target`, `take` and `close` already ask
    (`transport.governing_for`: `review.toml` at the stamped commit). A
    target this clone cannot resolve is a stated state, never a fallback to
    this checkout's rules or to a literal: the précis then says blocking
    cannot be judged and why.
    """
    from . import transport
    if not isinstance(sha, str) or not re.fullmatch(r"[0-9a-f]{40}", sha):
        return None, ("the verdict stamps no 40-character commit, so there "
                      "is no target whose taxonomy could be read")
    try:
        return transport.governing_for(cfg, sha), None
    except transport.Refusal as exc:
        return None, str(exc)


def _blocking_partition(findings, governing):
    """The précis's partition of `findings` under the TARGET's taxonomy:
    (blocking, downgraded, rest, outside).

    The membership rule is the validator's own — `validate.effective_blocking`,
    an exact match against the declared blocking list with the §5.3a
    downgrade — so what the précis files under Blocking is exactly what the
    disposition rules will treat as blocking, and nothing here case-folds,
    guesses or carries a severity name of its own. A finding whose severity
    the taxonomy does not declare is NOT non-blocking by default: it is its
    own group, because the validator refuses it (V-SEVERITY) and a précis
    that quietly filed it as advisory would describe a verdict that cannot
    be recorded.
    """
    from .validate import effective_blocking
    declared = set(governing.severities)
    blocking_set = set(governing.blocking_severities)
    blocking, downgraded, rest, outside = [], [], [], []
    for f in findings:
        if f.severity not in declared:
            outside.append(f)
        elif effective_blocking(f.severity, f.falsification, governing):
            blocking.append(f)
        elif f.severity in blocking_set:
            downgraded.append(f)
        else:
            rest.append(f)
    return blocking, downgraded, rest, outside


def verdict_precis(parsed, source: str | None = None, full: bool = False,
                   *, governing=None, unresolved: str | None = None) -> str:
    """What this verdict rules, in prose. Derived from the envelope, and
    from the TARGET's declared taxonomy for the one question the envelope
    cannot answer alone: which of its findings block.
    ...
    `governing` is the configuration at the verdict's target
    (`target_taxonomy`); `unresolved` says why there is none. There is NO
    default blocking set (0.25.0): absent a governing taxonomy the
    précis states that blocking cannot be judged and lists the findings
    ungrouped, rather than inventing a set that happens to match one
    repository's capitalisation.
    """
    ...
    if parsed.findings_none and not findings:
        ...
        if governing is None or not governing.taxonomy_declared:
            # No resolvable taxonomy: the count is stated and the question
            # it would answer is named as unanswerable, with the reason.
            why = (unresolved if governing is None else
                   "the target's configuration declares no taxonomy")
            why = _first_sentence(why or "no governing taxonomy was supplied")
            out.append(f"- **Findings** — {len(findings)}; blocking cannot "
                       f"be judged here — {why}")
            groups = [(findings, "**Findings — not partitioned: no target "
                                 "taxonomy to judge blocking by**")]
        else:
            blockers, downgraded, rest, outside = _blocking_partition(
                findings, governing)
            downgraded_ids = {f.id for f in downgraded}
            if not governing.blocking_severities:
                out.append(f"- **Findings** — {len(findings)}; the target "
                           f"declares no blocking severities, so none of "
                           f"them blocks by declaration")
            else:
                line = (f"- **Findings** — {len(findings)}, of which "
                        f"{len(blockers)} block")
                if downgraded:
                    line += (f"; {len(downgraded)} of blocking severity "
                             f"downgraded to advisory (no falsification "
                             f"test, §5.3a)")
                out.append(line)
            if outside:
                out.append(f"- **Severity not in the target's taxonomy** — "
                           f"{len(outside)} finding(s); the target declares "
                           f"{', '.join(governing.severities)}, and the "
                           f"validator refuses any other severity")
            groups = [
                (blockers, "**Blocking — these stop the change advancing**"),
                (downgraded + rest, "**Non-blocking**"),
                (outside, "**Severity not in the target's taxonomy — "
                          "neither blocking nor non-blocking until the "
                          "reviewer uses a declared severity**")]
        for group, heading in groups:
            ...
            out.append("")
            for f in group:
                note = (" (advisory by §5.3a: blocking severity, no "
                        "falsification test)"
                        if f.id in downgraded_ids else "")
                out.append(f"- **{f.id}** · {f.severity}{note} — "
                           + _wrap(f.title))
